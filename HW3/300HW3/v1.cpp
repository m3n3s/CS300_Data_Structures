...
void newEntryData(HashEntry<string, WordItem*>* entry, string file) // node = entry
{
	// edits the data of the existing entry in the table
		
	DocumentItem* p = entry->data->list;
	DocumentItem* ptr = p;
...
{
	// goes over every word and creates the tree structure
...
int main()
{
	...
	for(int i = 0; i < fileNumber; i++)
	{
		...
		cout << "succesfully preprocessed"<<endl;
	}
	...
	cout << "Speed up: " << BSTTime.count() / double(HTTime.count()) << endl;
