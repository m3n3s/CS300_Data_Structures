...
{
	// TO PRINT THE STACK IN CORRECT ORDER (FROM START TO FINSH)
	// RECURSIVE PRINT FUNCTION DID NOT WORK WITH LARGE MAZES SO I CHOSE THIS APPROACH
    ...
    { 
        mazeCell t = temp.top(); 
        out << t.col << " " << m - t.row - 1 << endl;
        temp.pop(); 

        p.push(t);   // To restore contents of the original stack. 
    } 	
}
